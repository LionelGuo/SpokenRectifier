...
/// The family's implementation spans this library's parts: the band
/// and fade models with the shared line geometry (slot_spans.dart),
/// the four painters (slot_painters.dart) and the TextInputClient
/// adapter mixin (slot_ime.dart) all share this library's scope.
library;
...
part 'slot_ime.dart';
part 'slot_painters.dart';
part 'slot_spans.dart';
...
/// The state's core (A3): everything but the IME adapter — the
/// capsule constants, the projection and composing paint-space seams,
/// both faces' builds, the input handling, the geometry and the test
/// seams. [SlotSurfaceState] below mixes the IME adapter (slot_ime.dart)
/// over this core.
abstract class _SlotSurfaceStateCore extends State<SlotSurface>
  ...
  // -- IME seams (slot_ime.dart) ------------------------------------------
  //
  // The adapter mixin implements these; the core's lifecycle and
  // mutation paths call them because the linearization hides mixin
  // members from core code.

  /// Dispose the platform connection and the focus listener (the
  /// state's dispose path).
  void _imeDispose();

  /// Drop the connection after a preview-round reset and re-open it
  /// when the focus still holds.
  void _imeResetConnection();

  /// Push the local state to the platform after a local mutation —
  /// the adapter's own _syncShadow, called by _afterLocalChange.
  void _syncShadow();
...
/// The concrete state: the core plus the IME adapter (slot_ime.dart's
/// [_SlotSurfaceIme] — the surface's [TextInputClient] face).
class SlotSurfaceState extends _SlotSurfaceStateCore with _SlotSurfaceIme {}
