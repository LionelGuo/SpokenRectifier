...
//! inserter, clock) is an injectable trait, so the whole deterministic test
//! suite and the `sr-replay` CLI driver run without network or audio
//! devices. Glossary terms live in the repository `CONTEXT.md`.
