//! The listening-pipe diagnostic record (26 号票): the microphone and
//! streaming-session facts land here, because the wedge this record
//! hunts is invisible everywhere else — a session that listens forever
//! with no events and no error leaves nothing on screen and nothing in
//! a console a GUI-subsystem exe does not have (oss-quality 07).
//!
//! The seam follows the insertion record's conventions (itself the perf
//! log's): the file resolves in the first writable search directory,
//! each run stamps a `--- run <time> ---` header, lines append one open
//! per call, a run start trims a record past 1 MiB, and a disk that
//! fails or vanishes retires the sink silently — diagnostics must never
//! be able to break the pipeline they observe. This copy lives in the
//! audio crate (the bottom of the listening-pipe dependency chain) so
//! both the microphone open and the ASR session pump write one file.
//!
//! What a line answers, per hypothesis of the no-text wedge:
//! mic-open wedges leave an `mic open` begin with no verdict (or a
//! timeout verdict); a VAD that never voices shows `voiced=0` against a
//! healthy `win_rms`; a device capturing digital silence shows a flat
//! near-zero `win_rms`; a dead server session shows `wire>0 svr=0`; a
//! half-open connection shows the `wire send stalled` fact.
...
/// neighbor of `spokenrectifier-insert.log` and the toml files.
pub const DIAG_LOG_FILE: &str = "spokenrectifier-asr.log";
...
impl DiagLog {
    /// A sink that records nowhere — tests.
    ...
    /// deleted file alive; an open per stats-cadence call is free).
...
/// The process-wide sink, opened lazily in the config search dirs the
/// first time the listening pipeline has a fact to record.
static GLOBAL: OnceLock<DiagLog> = OnceLock::new();

/// Record one fact line in the process-wide sink (a no-op when no
/// search directory is writable).
pub fn log(facts: &str) {
    GLOBAL
        .get_or_init(|| DiagLog::open_in(&spokenrectifier_config::search_dirs()))
        .log(facts);
}
...
mod tests {
    ...
    fn lines_append_with_the_run_header_above() {
        ...
        log.log("mic open ok 12ms 2ch 48000Hz");
        ...
        assert!(fact.contains("mic open ok"), "got: {fact}");
        assert!(fact.ends_with("48000Hz"), "got: {fact}");
