//! The scenario library's legacy TOML format: one file of named style
//! directives (glossary: 场景, 场景库, 风格指令). The library itself now
//! lives in the store's scenarios table (ADR-0023); this reader remains
//! as the migration's parse rules.
//!
//! A missing or unparsable file reads as an empty library: the loader
//! never errors and never writes.

use std::path::PathBuf;

/// The legacy library file's name, looked up in every config search
/// directory.
