//! The hotword dictionary's legacy text format: plain text, one term
//! per line. The dictionary itself now lives in the store's terms
//! table (ADR-0023); this reader remains as the migration's parse
//! rules and the read-only fallback for tools that run before the
//! store initializes.
//!
//! Lines are trimmed, blank lines and `#` comments are ignored; a
//! missing or unreadable file is simply an absent dictionary.

use std::path::PathBuf;

use crate::find_file;

/// The legacy dictionary file, git-ignored: it was the user's personal
/// terms.
